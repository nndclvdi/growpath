...
export default function TakeAssessment() {
  ...
  const handleNext = async () => {
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        // ==========================================
        // PERBAIKAN: Menggunakan URL Proxy ('/api/...')
        // ==========================================
        ...
        // Tangkap jika Backend menolak request (misal 401 atau 500)
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.message || 'Gagal menyimpan hasil assessment ke server');
        }
        ...
        const finalResult = {
          ...assessmentResult,
          // Gunakan optional chaining untuk mencegah error jika struktur responsenya berbeda
          attemptId: data?.data?.id?.toString() || new Date().getTime().toString(),
        };
        ...
        alert(`Failed submit assessment: ${error.message}`);
      }
    }
  };
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-xl font-bold text-slate-800">
            {assessmentData?.title || 'Skill Assessment'}
          </h2>
