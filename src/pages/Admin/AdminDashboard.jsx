...
const handleLogout = async () => {
  if (window.confirm("Apakah Anda yakin ingin keluar?")) {
    const success = await logout(); // Memanggil fungsi dari AppContext
    if (success) {
      navigate('/login-admin'); // Pindah halaman
    } else {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1400px] mx-auto pb-10">
      ...
      {/* HEADER SECTION DENGAN LOGOUT */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#181E2E] p-6 rounded-2xl border border-slate-700/50 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-white">Admin Central</h1>
          <p className="text-sm text-slate-400">Monitoring GrowPath talent & system metrics</p>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-bold text-white">{user?.name || 'Administrator'}</p>
            <p className="text-[11px] text-slate-500 uppercase tracking-wider font-bold">System Superadmin</p>
          </div>
          <button 
            onClick={handleLogout}
            className="flex items-center gap-2 px-5 py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl border border-red-500/20 transition-all font-bold text-sm"
          >
            <LogOut size={18} />
            Logout
          </button>
